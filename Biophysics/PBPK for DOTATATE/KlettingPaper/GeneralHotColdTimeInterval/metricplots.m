...
paramSweep(:,:,:,1) = paramSweep(:,:,:,1)*5; %note that since we had 5 injections so we should *5
paramSweep(:,:,:,2) = paramSweep(:,:,:,2)*5; %note that since we had 5 injections so we should *5
...
Y = normalizedTIA(:,:,:,1:4); %for OARs
x = normalizedTIA(:,:,:,5); %for tumor
X = x;
X(:,:,:,2)=x;
X(:,:,:,3)=x;
X(:,:,:,4)=x;
...
for organIndex=1:4
    organFigure = figure(organIndex); set(organFigure, "Position", ...
        [500+10*organIndex,500+10*organIndex,1050,600])
    ...
    for i=2:timeIntervalSteps
        ...
        caxis([-1.5,1.5]);
        %colorbar;
        title("time interval between injections: " + paramSweep(1,1,i,3) + " min", fontsize=14, FontWeight="normal");
    end

 
    cb = colorbar("Ticks",[-1,-0.5,0.5,1],"TickLabels",num2cell([-1,-0.5,0.5,1]));
    cb.Layout.Tile = 'east';
    cb.Label.String = "Efficacy";
    cb.Label.FontSize = 10;
    title(timeTiles, ["TIA in "+organsName(organIndex)+": 5 equal injections"], fontsize=15)
