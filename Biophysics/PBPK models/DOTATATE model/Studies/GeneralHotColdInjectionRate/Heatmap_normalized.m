...
for organIndex=1:5
    ...
    for i=1:timeIntervalSteps-1
        nexttile;
        contourf(paramSweep(:,:,i,paramChannelDictionary("Cold")), ...
            paramSweep(:,:,i,paramChannelDictionary("Hot")), ...
            TIA(:,:,i,organIndex)./TIA(:,:,timeIntervalSteps,organIndex), 7);
        colormap("jet")
%         caxis([min(min(min(min(TIA(:,:,:,:)./TIA(:,:,timeIntervalSteps,:))))), ...
%             max(max(max(max(TIA(:,:,:,:)./TIA(:,:,timeIntervalSteps,:)))))]);
        colorbar;
    ...
    title(timeTiles, ["NTIA in "+organsName(organIndex)+": constant rate infusion"], fontsize=15)
