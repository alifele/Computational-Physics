function [x, t, psi, psire, psiim, psimod, prob, v] = ...
    ...
    %% Evaluate idtype
        ...
        if idtype == 1
            ...
            delta = idpar(2);
            p = idpar(3);
            psi_t = exp(1i*p*xList) .* exp(-((xList-x0)/delta).^2);
        end
    ...
    %%% Calculate AB
        Diag = 1i - dt/2 * V;
        Ctilda = spdiags([Diag],[0],nx,nx);

        Diag = 1i + dt/2 * V;
        C = spdiags([Diag],[0],nx,nx);

        A = inv(Ctilda + R/2 * Delta);
        B = C - R/2 * Delta;
    ...
    %% Integrate and Calculate P
        psi_abs = abs(psiData).^2;
        integ_ = 0.5*(psi_abs + circshift(psi_abs,-1,2))*dx;
        integ = integ_(:,1:end-1);

        P = zeros(size(integ));
        summed = zeros(1,size(integ,1));
        for ii = [1:size(integ,2)]
            summed = summed + integ(:,ii).';
            P(:,ii) = summed ;
